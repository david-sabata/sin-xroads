...
public abstract class CustomWakerBehaviour extends SimpleBehaviour {
	...
	private boolean useFsmValue = false;


	/**
	 * Konstruktor ktery predpoklada ze je behaviour zapojeny do FSM.
	 * Zpozdeni pri kazdem zavolani je pak dane navratovou hodnotou 
	 * predchoziho stavu.
	 */
	public CustomWakerBehaviour(Agent a, boolean useFsmValue) {
		super(a);
		this.useFsmValue = useFsmValue;
		wakeupTime = -1;
		this.timeout = 1000;
		state = 0;
		finished = false;
	 ...
	 */
	public CustomWakerBehaviour(Agent a, Date wakeupDate) {
		super(a);
		timeout = 0;
	 ...
	 */
	public CustomWakerBehaviour(Agent a, long timeout) {
		super(a);
		wakeupTime = -1;
	...
	public void reset() {
		...
		// timeout podle posledniho stavu
		if (useFsmValue && parent instanceof FSMBehaviour) {
			FSMBehaviour fsm = (FSMBehaviour) parent;
			this.timeout = fsm.getLastExitValue();

			//			System.out.println("Krizovatka reset na novy timeout " + this.timeout);
		}
	}
